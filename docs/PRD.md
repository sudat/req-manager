...
| コンポーネント | 技術 | 選定理由 |
|---------------|------|---------|
| フロントエンド | Next.js 15 (App Router) | PRで指定された技術スタック |
...
実装を5つのフェーズに分け、段階的に機能を構築する。

| フェーズ | 目的 | 主な成果物 |
|---------|------|-----------|
| Phase 1 | 基盤構築 | プロジェクト設定、認証、DBスキーマ |
...
Phase 3とPhase 4は並行して進められる。Phase 5は両方の完了後に開始する。
...
プロジェクトのセットアップと認証・DBの基盤を構築する。

- [ ] 1-1. Next.js 15 (App Router) プロジェクト作成
- [ ] 1-2. Tailwind CSS + shadcn/ui セットアップ
- [ ] 1-3. Supabase プロジェクト作成・接続設定
- [ ] 1-4. BetterAuth セットアップ（メール認証）
...
- [ ] 1-9. RLS ポリシー設定（プロジェクト単位のアクセス制御）
...
| マイルストーン | 達成条件 | 想定期間 |
|--------------|---------|---------|
| M1: 基盤完成 | 認証、DB接続、共通レイアウトが動作 | 1週間 |
