...
### 1.4.1 ビジネスMVP時のコーディングエージェント運用

本ツールを実際のクライアントに提供し始めた初期段階（ビジネスMVP時）では、Claude Agent SDKは使用せず、開発者がClaude CodeやCodexを手動で操作して開発を行う。この段階では、改修指示パッケージを要件管理DBからエクスポートし、開発者がコーディングエージェントに手動で指示を出す運用とする。

コーディングエージェントは改修指示で不明点があれば、MCP経由で要件管理DBにアクセスし、改修指示で言及されていない情報を自己解決できる。

詳細は6.20節「ビジネスMVP時の運用方針」を参照。
...
coding_conventions:
  ...
  changeability:
    policy: pessimistic  # 変更可能性を悲観的に見積もる
    notes: |
      以下のカテゴリの値は「将来変更される可能性がある」と仮定し、
      ハードコードせず環境変数または設定テーブルに外出しすること。
      - 法的な数値（税率、手数料率等）
      - ビジネスルール上の閾値（上限金額、リトライ回数等）
      - 外部サービスのエンドポイント
      
      技術的な定数（配列サイズ、バッファ等）や物理・数学定数は
      ハードコードしてもよい。
```
...
業務要件は、業務タスクを達成するためにシステムや業務が「そうあってほしい」と言語化したものである。ケイパビリティ（～できること/できないこと）として記述し、その実現に際して守るべき制約を定義する。
...
goalはケイパビリティとして「何ができるようになるか」をMarkdownで記述する。constraintsはその能力を実現する際に「何を守らなければならないか」をYAML配列で列挙する。
...
### constraintsと独立BRの使い分け

制約（constraints）は同一BR内に記述するか、独立したBRとして切り出すかを判断する必要がある。以下の基準で使い分ける：

| 記述場所 | 基準 | 例 |
|---------|------|-----|
| **constraints内** | 同じオーナー、同じ変更頻度、局所的な制約 | PDFサイズ5MB以下、連番管理、日次締め切り |
| **独立BR** | 別オーナー、変更頻度が異なる、横断的影響、法的要件 | 適格請求書要件、与信確認、外部システム連携 |

**独立BRに切り出すべき典型的なケース：**

1. **法的・制度的な要件**
   - 税制改正等で頻繁に変更される可能性がある
   - 例：「適格請求書としての法的要件を満たす」

2. **別システム・別部門との連携**
   - 境界で変更が発生しやすい
   - 例：「与信状態を確認して安全な売上債権を確保する」

3. **変更頻度が著しく異なる**
   - 親BRは安定しているが、一部の制約が頻繁に変わる
   - 例：納品方法（郵送→メール→電子請求書）の変更

```yaml
# 良い例：独立BRとして切り出す
br_id: BR-BIL-001-01
goal: 請求書をPDF形式で出力できる
constraints:
  - ファイルサイズは5MB以下（メール添付制限）
  - 請求書番号は連番で管理し、欠番を避ける

---

br_id: BR-BIL-001-02
goal: 適格請求書としての法的要件を満たす
constraints:
  - 登録番号を記載すること
  - 税率ごとの対価と消費税額を区分して記載
  - 軽減税率対象品目は識別記号を付与
# 理由：税制改正対応で変更頻度が高いため独立
```
...
| ステータス | 意味 | 次のアクション |
|-----------|------|---------------|
| open | 起票済み、影響調査前 | 影響調査を開始する |
| review | 影響調査完了後のレビュー段階（疑義リンク解消など） | 改修指示パッケージを生成する |
| approved | 改修指示パッケージ生成済み（改修実行可能） | 改修実行/PR作成 |
| applied | 改修適用済み | （終了状態） |
...
| システム | 疑義リンクが未解消の場合、改修指示パッケージ生成をブロック（statusはreviewのまま） |
...
変更対応フローは5ステップで構成されるため、ユーザーが「現在どのステップにいるか」「次に何をすべきか」を常に把握できるようにする。

| ガイダンス要素 | 目的 |
|---------------|------|
| プログレス表示 | 全体の進捗と各ステップの完了状態を視覚的に表示する |
| アクションガイダンス | 各ステータスでプライマリアクション（次に進むボタン）を明示する |
| 完了条件の表示 | 次のステップに進むために必要な条件をリスト表示する |
| 遷移プレビュー | 次のステップで何が起きるかを事前に説明する |

ステータスごとの主要アクション例：

| ステータス | プライマリアクション | 完了条件 |
|-----------|---------------------|---------|
| open | 「影響調査を開始する」 | CRの内容が入力されている |
| review | 「改修指示パッケージ生成」 | 疑義リンクが未解消の場合はブロックされる |
| approved | 「改修ジョブを投入」 | パッケージが生成されている |
| applied | - | （終了状態） |

具体的なUI設計（レイアウト、コンポーネント、インタラクション）は実装フェーズで定義する。
...
- **6.20 ビジネスMVP時の運用方針**: クライアント獲得後の手動運用とPMF後の自動化
...
本ツールを実際のクライアントに提供し始めた初期段階では、以下の運用方針とする：

**コーディングエージェントの利用方法：**
- Claude Agent SDKは使用しない
- 開発者がClaude Code / Codexを手動で操作
- 改修指示パッケージは要件管理DBのエクスポート機能からダウンロード
- **推奨**: ダウンロードしたファイルを `docs/instruction-packages/{cr-id}.md` に配置し、Claude Codeは `@docs/instruction-packages/` で参照

**MCP Serverの役割：**
- コーディングエージェントが改修指示で不明点がある場合、MCP経由で要件管理DBにアクセス
- 改修指示で言及されていない情報を自己解決
- 例：エントリポイントから関連要件を逆引き、影響範囲を補完

### PMF確認後（サービス成長期）

クライアントからのフィードバックを基にPMF（Product-Market Fit）を確認した後、以下を検討：
- Claude Agent SDK等を活用した改修の完全自動化
- 改修指示パッケージの自動生成と自動実行
- コーディングエージェントへの直接ジョブ投入
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  status: open (investigation_results: running)                       │
│                                                                     │
│  [概要] [影響調査] [疑義リンク] [改修指示]                          │
│  ─────────────────────────────────────────────────────────────────  │
│                                                                     │
...
#### 議事録から草案（Minutes intake）

議事録/ヒアリングメモを貼り付けて、複数の業務タスク（BT）/業務要件（BR）草案をまとめて生成する導線。

- `/chat` ヘッダーの「議事録から草案」から起動
- ステップ1: 議事録テキスト貼り付け → 候補抽出（`POST /api/intake/minutes/extract`）
- ステップ2: 候補ごとに業務領域（BD）を割り当て → BT/BR草案一括生成（`POST /api/intake/minutes/generate`）
- 生成された草案は既存の草案カードで表示し、確定（正本登録）はカード単位で行う（バッチ確定はしない）
...
| Phase 5 | 変更管理と連携 | CR、影響調査、エクスポート、MCP Server（最小限） |
...
- [x] 1-1. Next.js 16 (App Router) プロジェクト作成
- [x] 1-2. Tailwind CSS + shadcn/ui セットアップ
...
- [x] 2-1. プロジェクト一覧・作成画面
- [x] 2-2. プロダクト要件（PR）編集画面（基本情報タブ）
- [x] 2-3. PR編集画面（技術スタック・コーディング規約タブ）
- [x] 2-4. 業務領域（BD）一覧・作成・編集画面
- [x] 2-5. 業務タスク（BT）詳細・作成・編集画面
- [x] 2-6. 業務要件（BR）作成・編集フォーム（BT詳細画面内）
- [x] 2-7. システム領域（SD）一覧・作成・編集画面
...
- [x] 3-1. requirement_links テーブル操作API
- [x] 3-2. realizesリンク作成UI（BR詳細画面でSFを選択・リンク）
- [x] 3-3. リンク一覧表示（SF詳細画面で「実現する業務要件」を表示）
- [x] 3-4. 概念辞書一覧・作成・編集画面
- [x] 3-5. 要件へのconcept_idsリンク機能
- [x] 3-6. ヘルススコア計算ロジック（構造チェック）
- [x] 3-7. ヘルススコア計算ロジック（データ品質チェック）
- [x] 3-8. ヘルススコアダッシュボード表示
...
#### 未実装項目（将来実装予定）
- 5-8. MCP Server（完全版） - APIキー認証、レート制限、監査ログ
- 5-9. 設計決定ログ記録機能 - Phase 6以降で検討
...
| SuggestionFromAgent | コーディングエージェントから要件管理DBアプリへの改善提案 |
| SyncCheckRequest | コードと正本の同期性をチェックするリクエスト |
