...
- [ ] スコアリング仕様の確定（対象エンティティ/項目/重み/閾値/表示ルール）
- [ ] 欠損・不整合の検出ルール確定（例: `responsibility` null/空、受入条件0件など）
- [ ] スコア計算ロジック実装（DRY: 共通の集計/判定関数に寄せる）
- [ ] 算出場所の決定（KISS: まずはアプリ側で算出 → 必要ならDB集計/RPCへ）
- [ ] UI表示（一覧/カードにスコア・警告バッジ、詳細に内訳）
- [ ] 最小テスト（スコア関数の単体 or スモーク）
