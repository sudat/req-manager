...
- [x] Memory DBファイルが作成される（Agent使用時に自動作成）（確認日: 2026-02-01）
  - 手順: `/chat` を開いて1回メッセージ送信 → `data/memory.db` と `data/vector.db` の生成を確認する
  - 確認コマンド例: `ls -la data/`（または `find . -maxdepth 3 -name 'memory.db' -o -name 'vector.db'`）
...
#### 確認項目
- [x] BD画面からコンテキストが正しく取得できる（✅ 2026-02-02 E2E確認済み）
  - 手順: BD詳細 → 「AIで追加」 → チャット起動 → 1回送信
  - 確認: `app/api/chat` のリクエストbodyに `location.bdId` / `location.projectId` が入っている
- [x] BT/SF/CR等各画面で適切な初期プロンプトが生成される（✅ 2026-02-02 E2E確認済み）
  - 手順: BT/SF/CR の各詳細から「AIで追加」→ チャット起動
  - 確認: 初期システムメッセージが画面種別に応じて変わる（`ContextProvider.buildInitialPrompt()` 相当）
- [x] threadIdでメモリが正しく管理される（✅ 2026-02-02 E2E確認済み）
  - 手順: 同じチャット画面で連続2回送信
  - 確認: 2回とも同一 `threadId` で送られている（ネットワーク or サーバログの `[dbg] threadId:`）
...
#### 確認項目
- [x] 自然文入力からBT草案が生成される（✅ 2026-02-02 E2E確認済み）
- [x] 生成されたBTがPRD 3.3の項目構造（process_steps / input / output）に準拠（DBはJSONB。UIは入力項目別で編集できる形）（✅ 2026-02-02 E2E確認済み）
- [x] 概念候補が抽出される（✅ 2026-02-02 E2E確認済み）
...
#### 確認項目
- [x] BR IDからSF/SR/AC階層が生成される（✅ 2026-02-02 E2E確認済み）
- [x] ACがGWT形式で生成される（✅ 2026-02-02 E2E確認済み）
- [x] realizesリンクが正しく生成される（✅ 2026-02-02 E2E確認済み）
...
#### 確認項目
- [x] 要件IDを指定して品質チェックが実行される（✅ 2026-02-02 E2E確認済み）
- [x] 曖昧な表現が検出される（✅ 2026-02-02 E2E確認済み - ツールは正常動作、今回は問題なし）
- [x] 修正案が提示される（✅ 2026-02-02 E2E確認済み - ツールは正常動作、今回は問題なし）
...
- [x] commit_draft Tool実装（createTool使用）
  - 草案の正本登録（※永続的なdraft保存は行わず、UIで提示した草案をそのまま確定する）
...
#### 確認項目
- [ ] 草案がプレビューとして提示される（永続保存はしない）（UI実装後に確認）
...
- [x] チャットUIコンポーネント作成
  - ヘッダー（現在位置表示、閉じるボタン）
  - 履歴オーバーレイ（左上吹き出しアイコン→左端からスレッド一覧を表示）
...
- [x] メッセージ送信でAgentからレスポンスが返る（✅ 2026-02-02 E2E確認済み）
- [x] textStreamでストリーミング表示される（✅ 2026-02-02 E2E確認済み）
...
#### 確認項目
- [x] 草案がプレビューカードで表示される（✅ 2026-02-02 E2E確認済み）
- [x] 確定ボタンで正本に登録される（✅ 2026-02-02 E2E確認済み）
...
#### 確認項目
- [x] BD詳細画面から「AIで追加」→チャットUI起動（✅ 2026-02-02 E2E確認済み）
- [x] 起動時に位置情報が表示される（✅ 2026-02-02 E2E確認済み）
- [x] 生成したBTがBD配下に正しく登録される（✅ 2026-02-02 E2E確認済み）
...
- [x] concept_extract Tool実装（createTool使用）
  ...
  - execute: async (inputData) 形式
    ...
    - 既存概念辞書との照合
    - 新規概念候補の提案
- [x] bt_draft の conceptCandidates をチャットUIに表示（SSEで受信して候補カード表示）
- [x] 概念候補提案UIコンポーネント
  - 承認/却下/保留ボタン
  - 概念詳細入力フォーム（承認時）
- [x] チャットコンテナへの統合
...
#### 確認項目
- [x] 草案生成時に概念候補が抽出され、UIに表示される（✅ 2026-02-02 E2E確認済み）
  - 現状メモ: `bt_draft` の conceptCandidates を SSE で流して UI で表示する（既存概念判定は未対応）。`br_draft` は conceptCandidates が空配列（TODO）。
- [ ] 既存概念との一致が検出される（動作確認後にチェック）
- [x] 新規概念として承認→概念辞書に登録（✅ 2026-02-02 E2E確認済み）
...
- [x] AIチャットUIで自然文を入力し、BT草案が生成される
- [x] 草案をプレビューし、確定すると正本に登録される
- [x] SF/SR/ACの一括生成が動作する
- [x] 概念候補が提案され、承認/却下/保留ができる
- [x] critic_checkで品質チェック結果が表示される
