...
| 構造 | 業務 → 業務要件 → システム要件 | 変更要求 → 影響要件 → 変更内容 |
...
- 入力：自然文での質問（テキスト/音声）
- 出力：関連する業務・業務要件・システム要件・概念・変更要求をランキング表示
- 各結果に「なぜ関連するか」の根拠を添付（クリックで原文へ遷移）
- 結果から業務詳細、要件詳細、変更要求詳細へ直接遷移可能
...
- 操作：照会→業務詳細（照会）、修正→業務詳細（更新）、削除→確認後削除
