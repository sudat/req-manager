...
import type { TaskKnowledge, Requirement, DesignDoc, CodeRef } from "@/lib/mock/task-knowledge";
...
export default function TaskDetailEditPage({
	...
	const updateDesignDoc = (docId: string, patch: Partial<DesignDoc>) => {
		setKnowledge((prev) => ({
			...prev,
			designDocs: prev.designDocs.map((d) => (d.id === docId ? { ...d, ...patch } : d)),
		}));
	};

	const addDesignDoc = () => {
		setKnowledge((prev) => {
			const id = nextSequentialId(`DD-${taskId}`, prev.designDocs.map((d) => d.id));
			return {
				...prev,
				designDocs: [
					...prev.designDocs,
	...
	const removeDesignDoc = (docId: string) => {
		setKnowledge((prev) => ({
			...prev,
			designDocs: prev.designDocs.filter((d) => d.id !== docId),
		}));
	};

	const updateCodeRefPaths = (refIndex: number, paths: string[]) => {
		setKnowledge((prev) => ({
			...prev,
			codeRefs: prev.codeRefs.map((r, i) => (i === refIndex ? { ...r, paths } : r)),
		}));
	};

	const addPathToCodeRef = (refIndex: number) => {
		setKnowledge((prev) => ({
			...prev,
			codeRefs: prev.codeRefs.map((r, i) =>
				i === refIndex ? { ...r, paths: [...r.paths, ""] } : r
			),
		}));
	};

	const removePathFromCodeRef = (refIndex: number, pathIndex: number) => {
		setKnowledge((prev) => ({
			...prev,
			codeRefs: prev.codeRefs.map((r, i) =>
				i === refIndex ? { paths: r.paths.filter((_, j) => j !== pathIndex) } : r
			),
		}));
	};
	...
	const addCodeRef = () => {
		setKnowledge((prev) => ({
			...prev,
			codeRefs: [...prev.codeRefs, { paths: [] }],
		}));
	};

	const removeCodeRef = (refIndex: number) => {
		setKnowledge((prev) => ({
			...prev,
			codeRefs: prev.codeRefs.filter((_, i) => i !== refIndex),
		}));
	};
				...
				<div className="mx-auto max-w-[1400px] p-8">
					...
					<Card className="mt-6">
						<CardHeader className="flex flex-row items-center justify-between">
							...
							<Button variant="outline" size="sm" className="gap-2" onClick={addDesignDoc}>
								<Plus className="h-4 w-4" />
								追加
							</Button>
						</CardHeader>
						<CardContent className="space-y-4">
							{knowledge.designDocs.length === 0 ? (
								...
								knowledge.designDocs.map((doc) => (
									<Card key={doc.id} className="border-slate-100">
										<CardHeader className="flex flex-row items-start justify-between gap-3">
											<div>
											...
											<Button variant="outline" size="icon" title="削除" onClick={() => removeDesignDoc(doc.id)}>
												<Trash2 className="h-4 w-4 text-rose-500" />
											</Button>
										</CardHeader>
										<CardContent className="space-y-4">
											<div className="space-y-2">
												<Label>タイトル</Label>
												<Input value={doc.title} onChange={(e) => updateDesignDoc(doc.id, { title: e.target.value })} />
											</div>
											<div className="grid gap-4 md:grid-cols-2">
												<div className="space-y-2">
													<Label>カテゴリ（画面/データ/IF/業務ルール/その他）</Label>
													<Input
														value={doc.category}
														onChange={(e) =>
															updateDesignDoc(doc.id, { category: (e.target.value || "その他") as DesignDoc["category"] })
														}
													/>
												</div>
												<div className="space-y-2">
													<Label>ソース（内部/外部リンク）</Label>
													<Input
														value={doc.source}
														onChange={(e) =>
															updateDesignDoc(doc.id, { source: (e.target.value || "内部") as DesignDoc["source"] })
														}
													/>
												</div>
											</div>
											<div className="space-y-2">
												<Label>外部URL（任意）</Label>
												<Input value={doc.url ?? ""} onChange={(e) => updateDesignDoc(doc.id, { url: e.target.value })} />
											</div>
											<div className="space-y-2">
												<Label>本文（Markdown可）</Label>
												<Textarea
													className="min-h-[160px]"
													value={doc.content}
													onChange={(e) => updateDesignDoc(doc.id, { content: e.target.value })}
												/>
											</div>
										</CardContent>
									</Card>
					...
					<Card className="mt-6">
						...
						<CardContent className="space-y-4">
							{knowledge.codeRefs.length === 0 ? (
								...
								knowledge.codeRefs.map((ref, refIndex) => (
									<Card key={refIndex} className="border-slate-100">
										<CardHeader className="flex flex-row items-start justify-between gap-3">
											<div></div>
											{ref.paths.length > 1 && (
												<Button variant="outline" size="icon" title="削除" onClick={() => removeCodeRef(refIndex)}>
													<Trash2 className="h-4 w-4 text-rose-500" />
												</Button>
											)}
										</CardHeader>
										<CardContent className="space-y-3">
											{ref.paths.map((path, pathIndex) => (
												path && (
													...
													<div className="flex-1">
														<Input
															...
															onChange={(e) => {
																...
																const newPaths = ref.paths.map((p, i) => i === pathIndex ? newValue : p);
																updateCodeRefPaths(refIndex, newPaths);
															}}
														/>
													</div>
														{ref.paths.length > 1 && (
															<Button
																...
																size="icon"
																title="削除"
																onClick={() => removePathFromCodeRef(refIndex, pathIndex)}
															>
																<Trash2 className="h-4 w-4 text-rose-500" />
															</Button>
														)}
											...
											<Button
												...
												size="sm"
												className="gap-2"
												onClick={() => addPathToCodeRef(refIndex)}
											>
												<Plus className="h-4 w-4" />
												パスを追加
											</Button>
