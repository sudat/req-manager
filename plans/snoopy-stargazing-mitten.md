...
3. 任意のドメインを選択
4. `/system/{id}/functions` でシステム機能一覧が表示されることを確認
5. `/system/{id}/functions/create` にアクセス
6. システム機能作成フォームが表示されることを確認
7. 必須項目の入力フィールドが存在することを確認

### シナリオ4: システム要件の照会・追加
1. システム機能一覧から任意の機能を選択
2. システム要件詳細ページ `/system/{id}/{srfId}` が表示されることを確認
3. システム要件セクションが正しく表示されていることを確認
4. 編集ページ `/system/{id}/{srfId}/edit` にアクセス
5. 編集フォームが表示されることを確認
