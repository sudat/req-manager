import { useState, type ReactNode } from "react";
import { Plus, Trash2, X } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Textarea } from "@/components/ui/textarea";
import type { BusinessRule } from "@/lib/domain/schemas/core-logic";
import { FoldableStructuredSection } from "../FoldableStructuredSection";
import { BUSINESS_RULE_TYPE_LABELS, BUSINESS_RULE_TYPES } from "../constants";
import { removeAtIndex, updateAtIndex } from "@/lib/utils/array-updates";
import {
  getSideEffectKindLabel,
  type RuleSideEffectKind,
} from "@/lib/utils/design-documents/side-effect-rule-link";
import type { StructuredSpecEditorProps } from "./types";

interface CoreLogicEditorProps {
  spec: StructuredSpecEditorProps["spec"];
  updateStructuredSpec: StructuredSpecEditorProps["updateStructuredSpec"];
}

const EMPTY_SELECT_VALUE = "__none__";

type RuleSequence = NonNullable<BusinessRule["sequence"]>;
type PreconditionViolation = NonNullable<BusinessRule["preconditionViolations"]>[number];
function normalizeRuleSequence(
  sequence: Partial<RuleSequence>
): RuleSequence | undefined {
  if (!sequence.fragmentType) return undefined;

  const normalized: Partial<RuleSequence> = {
    fragmentType: sequence.fragmentType,
  };

  if (sequence.fragmentGroup && sequence.fragmentGroup.trim().length > 0) {
    normalized.fragmentGroup = sequence.fragmentGroup.trim();
  }
  if (sequence.fragmentType === "alt" && sequence.branch) {
    normalized.branch = sequence.branch;
  }
  if (sequence.guard && sequence.guard.trim().length > 0) {
    normalized.guard = sequence.guard.trim();
  }
  if (
    sequence.fragmentType === "loop" &&
    sequence.loopLabel &&
    sequence.loopLabel.trim().length > 0
  ) {
    normalized.loopLabel = sequence.loopLabel.trim();
  }

  return normalized as RuleSequence;
}

function patchRuleSequence(
  current: BusinessRule["sequence"],
  patch: Partial<RuleSequence>
): RuleSequence | undefined {
  return normalizeRuleSequence({
    ...(current ?? {}),
    ...patch,
  });
}

function findPreconditionViolation(
  violations: BusinessRule["preconditionViolations"],
  preconditionIndex: number
): PreconditionViolation | undefined {
  return (violations ?? []).find((item) => item.preconditionIndex === preconditionIndex);
}

function setPreconditionViolation(
  violations: BusinessRule["preconditionViolations"],
  preconditionIndex: number,
  exceptionIndex?: number
): BusinessRule["preconditionViolations"] {
  const withoutTarget = (violations ?? []).filter(
    (item) => item.preconditionIndex !== preconditionIndex
  );

  if (exceptionIndex === undefined) {
    return withoutTarget.length > 0 ? withoutTarget : undefined;
  }

  const next = [
    ...withoutTarget,
    {
      preconditionIndex,
      exceptionIndex,
    },
  ].sort((a, b) => a.preconditionIndex - b.preconditionIndex);

  return next;
}

function removePreconditionAndShiftViolations(
  preconditions: BusinessRule["preconditions"],
  violations: BusinessRule["preconditionViolations"],
  removedPreconditionIndex: number
): Pick<BusinessRule, "preconditions" | "preconditionViolations"> {
  const nextPreconditions = removeAtIndex(preconditions, removedPreconditionIndex);
  const nextViolations = (violations ?? [])
    .filter((item) => item.preconditionIndex !== removedPreconditionIndex)
    .map((item) =>
      item.preconditionIndex > removedPreconditionIndex
        ? { ...item, preconditionIndex: item.preconditionIndex - 1 }
        : item
    );

  return {
    preconditions: nextPreconditions,
    preconditionViolations: nextViolations.length > 0 ? nextViolations : undefined,
  };
}

export function CoreLogicEditor({ spec, updateStructuredSpec }: CoreLogicEditorProps): ReactNode {
  const [openedViolationEditors, setOpenedViolationEditors] = useState<Record<string, boolean>>({});
  const [summaryFocused, setSummaryFocused] = useState(false);
  const [ruleDescFocused, setRuleDescFocused] = useState<Record<number, boolean>>({});

  if (spec.ioType === "model") {
    return null;
  }

  const exceptionOptions = spec.exceptions.map((exception, index) => {
    const errorCode = exception.errorCode?.trim();
    const condition = exception.condition?.trim();
    const message = exception.message?.trim();
    const label = errorCode && errorCode.length > 0 ? errorCode : `例外${index + 1}`;
    const detail = condition && condition.length > 0
      ? condition
      : message && message.length > 0
        ? message
        : exception.type;
    return {
      index,
      label,
      detail,
    };
  });

  const setSideEffectRuleRef = (
    kind: RuleSideEffectKind,
    index: number,
    ruleRef: string | undefined
  ) => {
    updateStructuredSpec((current) => {
      if (kind === "db") {
        return {
          ...current,
          sideEffects: {
            ...current.sideEffects,
            dbOperations: updateAtIndex(current.sideEffects.dbOperations, index, { ruleRef }),
          },
        };
      }
      if (kind === "api") {
        return {
          ...current,
          sideEffects: {
            ...current.sideEffects,
            externalApiCalls: updateAtIndex(current.sideEffects.externalApiCalls, index, { ruleRef }),
          },
        };
      }
      if (kind === "event") {
        return {
          ...current,
          sideEffects: {
            ...current.sideEffects,
            events: updateAtIndex(current.sideEffects.events, index, { ruleRef }),
          },
        };
      }

      return {
        ...current,
        sideEffects: {
          ...current.sideEffects,
          fileOutputs: updateAtIndex(current.sideEffects.fileOutputs, index, { ruleRef }),
        },
      };
    });
  };

  return (
    <FoldableStructuredSection
      title="コアロジック"
      description="入力から出力への処理ルールを定義します"
      titleTooltip="入力データを出力データに変換する際に適用されるビジネスルール（検証、抽出、算出、判定、永続化）を記述します。"
      defaultOpen={false}
    >
      <div className="space-y-4">
        <div className="space-y-2">
          <Label className="text-[11px] text-slate-500">全体概要</Label>
          <Textarea
            value={spec.coreLogic.summary || ""}
            onChange={(e) =>
              updateStructuredSpec((current) => ({
                ...current,
                coreLogic: { ...current.coreLogic, summary: e.target.value },
              }))
            }
            onFocus={() => setSummaryFocused(true)}
            onBlur={() => setSummaryFocused(false)}
            placeholder="コアロジック全体の概要説明"
            className="text-sm min-h-[32px] resize-y transition-[min-height]"
            style={{ minHeight: summaryFocused ? 56 : 32 }}
          />
        </div>

        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label className="text-[12px] font-medium text-slate-500">ビジネスルール</Label>
            <Button
              type="button"
              variant="default"
              size="sm"
              className="h-7 gap-2 text-[12px]"
              onClick={() => {
                const newRule: BusinessRule = {
                  name: "",
                  type: "validate",
                  description: "",
                };
                updateStructuredSpec((current) => ({
                  ...current,
                  coreLogic: {
                    ...current.coreLogic,
                    rules: [...(current.coreLogic.rules || []), newRule],
                  },
                }));
              }}
            >
              <Plus className="h-4 w-4" />
              追加
            </Button>
          </div>

          {spec.coreLogic.rules && spec.coreLogic.rules.length > 0 ? (
            <div className="space-y-3">
              {spec.coreLogic.rules.map((rule, ruleIndex) => {
                const normalizedRuleName = rule.name?.trim();
                const sideEffectSelections: Array<{
                  key: string;
                  kind: RuleSideEffectKind;
                  index: number;
                  label: string;
                  ruleRef?: string;
                }> = [
                  ...(spec.sideEffects.dbOperations ?? []).map((operation, index) => ({
                    key: `db-${index}`,
                    kind: "db" as const,
                    index,
                    label: `${operation.operation.toUpperCase()} ${operation.table || "(table未設定)"}`,
                    ruleRef: operation.ruleRef?.trim(),
                  })),
                  ...(spec.sideEffects.externalApiCalls ?? []).map((apiCall, index) => ({
                    key: `api-${index}`,
                    kind: "api" as const,
                    index,
                    label: `${apiCall.method} ${apiCall.endpoint || "(endpoint未設定)"}`,
                    ruleRef: apiCall.ruleRef?.trim(),
                  })),
                  ...(spec.sideEffects.events ?? []).map((event, index) => ({
                    key: `event-${index}`,
                    kind: "event" as const,
                    index,
                    label: `EVENT ${event.eventType || "(eventType未設定)"}`,
                    ruleRef: event.ruleRef?.trim(),
                  })),
                  ...(spec.sideEffects.fileOutputs ?? []).map((fileOutput, index) => ({
                    key: `file-${index}`,
                    kind: "file" as const,
                    index,
                    label: `FILE ${fileOutput.format} ${fileOutput.path || "(path未設定)"}`,
                    ruleRef: fileOutput.ruleRef?.trim(),
                  })),
                ];
                const linkedSideEffectCount = normalizedRuleName
                  ? sideEffectSelections.filter((item) => item.ruleRef === normalizedRuleName).length
                  : 0;

                return (
                  <div
                    key={ruleIndex}
                    className="rounded-md border border-slate-200 p-3 space-y-3"
                  >
                  <div className="flex items-center justify-between">
                    <Badge variant={rule.type}>
                      {`ルール${ruleIndex + 1} : ${normalizedRuleName || "未設定"}`}
                    </Badge>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
                        updateStructuredSpec((current) => ({
                          ...current,
                          coreLogic: {
                            ...current.coreLogic,
                            rules: removeAtIndex(current.coreLogic.rules, ruleIndex),
                          },
                        }));
                      }}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  </div>

                  <div className="overflow-x-auto pb-1">
                    <div className="grid w-full min-w-[760px] gap-3 grid-cols-[20%_15%_1fr] items-start">
                      <div className="space-y-1">
                        <Label className="text-[11px] text-slate-500">ルール名</Label>
                        <Input
                          value={rule.name}
                          onChange={(e) => {
                            updateStructuredSpec((current) => ({
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  name: e.target.value,
                                }),
                              },
                            }));
                          }}
                          placeholder="例: tax_calculation"
                          className="h-8 text-sm"
                        />
                      </div>

                      <div className="space-y-1">
                        <Label className="text-[11px] text-slate-500">種別</Label>
                        <Select
                          value={rule.type}
                          onValueChange={(value) => {
                            updateStructuredSpec((current) => ({
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  type: value as BusinessRule["type"],
                                }),
                              },
                            }));
                          }}
                        >
                          <SelectTrigger className="h-8 w-full text-sm">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            {BUSINESS_RULE_TYPES.map((type) => (
                              <SelectItem key={type} value={type}>
                                {BUSINESS_RULE_TYPE_LABELS[type]}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>

                      <div className="space-y-1">
                        <Label className="text-[11px] text-slate-500">説明</Label>
                        <Textarea
                          value={rule.description}
                          onChange={(e) => {
                            updateStructuredSpec((current) => ({
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  description: e.target.value,
                                }),
                              },
                            }));
                          }}
                          onFocus={() => setRuleDescFocused((prev) => ({ ...prev, [ruleIndex]: true }))}
                          onBlur={() => setRuleDescFocused((prev) => ({ ...prev, [ruleIndex]: false }))}
                          placeholder="ルールの説明"
                          className="text-sm min-h-[32px] resize-y transition-[min-height]"
                          style={{ minHeight: ruleDescFocused[ruleIndex] ? 56 : 32 }}
                        />
                      </div>
                    </div>
                  </div>

                  <Tabs defaultValue="formulas" className="space-y-2">
                    <TabsList className="grid h-8 w-full grid-cols-5">
                      <TabsTrigger value="formulas" className="text-xs">
                        計算式・判定式
                      </TabsTrigger>
                      <TabsTrigger value="preconditions" className="text-xs">
                        前提条件
                      </TabsTrigger>
                      <TabsTrigger value="sideEffects" className="text-xs">
                        副作用
                      </TabsTrigger>
                      <TabsTrigger value="notes" className="text-xs">
                        補足事項
                      </TabsTrigger>
                      <TabsTrigger value="sequence" className="text-xs">
                        シーケンス制御
                      </TabsTrigger>
                    </TabsList>

                    <TabsContent value="formulas" className="space-y-2">
                      {(rule.formulas || []).map((formula, formulaIndex) => (
                        <div key={formulaIndex} className="flex items-center gap-2">
                          <Input
                            value={formula}
                            onChange={(e) => {
                              updateStructuredSpec((current) => ({
                                ...current,
                                coreLogic: {
                                  ...current.coreLogic,
                                  rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                    formulas: updateAtIndex(
                                      (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.formulas,
                                      formulaIndex,
                                      e.target.value
                                    ),
                                  }),
                                },
                              }));
                            }}
                            placeholder="例: 税額 = 税抜金額 × 税率"
                          />
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => {
                              updateStructuredSpec((current) => ({
                                ...current,
                                coreLogic: {
                                  ...current.coreLogic,
                                  rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                    formulas: removeAtIndex(
                                      (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.formulas,
                                      formulaIndex
                                    ),
                                  }),
                                },
                              }));
                            }}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      ))}
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          updateStructuredSpec((current) => {
                            const currentFormulas = (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.formulas ?? [];
                            return {
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  formulas: [...currentFormulas, ""],
                                }),
                              },
                            };
                          });
                        }}
                      >
                        <Plus className="mr-1 h-3 w-3" />
                        式を追加
                      </Button>
                    </TabsContent>

                    <TabsContent value="preconditions" className="space-y-2">
                      {(rule.preconditions || []).map((precondition, preconditionIndex) => {
                        const violation = findPreconditionViolation(
                          rule.preconditionViolations,
                          preconditionIndex
                        );
                        const editorKey = `${ruleIndex}:${preconditionIndex}`;
                        const opened = openedViolationEditors[editorKey] || Boolean(violation);

                        return (
                          <div key={preconditionIndex} className="overflow-x-auto pb-1">
                            <div className="flex min-w-[980px] items-center gap-2">
                              <Input
                                value={precondition}
                                onChange={(e) => {
                                  updateStructuredSpec((current) => ({
                                    ...current,
                                    coreLogic: {
                                      ...current.coreLogic,
                                      rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                        preconditions: updateAtIndex(
                                          (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.preconditions,
                                          preconditionIndex,
                                          e.target.value
                                        ),
                                      }),
                                    },
                                  }));
                                }}
                                placeholder="例: 税抜金額 > 0"
                                className="h-8 text-sm flex-1"
                              />
                              <Button
                                variant="outline"
                                size="sm"
                                className="h-8 shrink-0"
                                onClick={() => {
                                  const key = `${ruleIndex}:${preconditionIndex}`;
                                  setOpenedViolationEditors((current) => ({
                                    ...current,
                                    [key]: !current[key],
                                  }));
                                }}
                              >
                                違反時の挙動
                              </Button>

                              {opened &&
                                (exceptionOptions.length > 0 ? (
                                  <div className="flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 shrink-0">
                                    <span className="text-[11px] text-slate-600">例外</span>
                                    <Select
                                      value={
                                        violation
                                          ? String(violation.exceptionIndex)
                                          : EMPTY_SELECT_VALUE
                                      }
                                      onValueChange={(value) => {
                                        updateStructuredSpec((current) => {
                                          const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                          const nextViolations = setPreconditionViolation(
                                            currentRule.preconditionViolations,
                                            preconditionIndex,
                                            value === EMPTY_SELECT_VALUE ? undefined : Number(value)
                                          );
                                          return {
                                            ...current,
                                            coreLogic: {
                                              ...current.coreLogic,
                                              rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                                preconditionViolations: nextViolations,
                                              }),
                                            },
                                          };
                                        });
                                      }}
                                    >
                                      <SelectTrigger className="h-8 w-[320px]">
                                        <SelectValue placeholder="例外を選択" />
                                      </SelectTrigger>
                                      <SelectContent>
                                        <SelectItem value={EMPTY_SELECT_VALUE}>未設定</SelectItem>
                                        {exceptionOptions.map((option) => (
                                          <SelectItem key={option.index} value={String(option.index)}>
                                            {`${option.label}: ${option.detail}`}
                                          </SelectItem>
                                        ))}
                                      </SelectContent>
                                    </Select>
                                  </div>
                                ) : (
                                  <div className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs text-muted-foreground shrink-0">
                                    例外セクションで先に例外を追加してください
                                  </div>
                                ))}

                              <Button
                                variant="ghost"
                                size="icon"
                                className="shrink-0"
                                onClick={() => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    const next = removePreconditionAndShiftViolations(
                                      currentRule.preconditions,
                                      currentRule.preconditionViolations,
                                      preconditionIndex
                                    );
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, next),
                                      },
                                    };
                                  });
                                  setOpenedViolationEditors((current) => {
                                    const key = `${ruleIndex}:${preconditionIndex}`;
                                    if (!current[key]) return current;
                                    const next = { ...current };
                                    delete next[key];
                                    return next;
                                  });
                                }}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
                          </div>
                        );
                      })}
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          updateStructuredSpec((current) => {
                            const currentPreconditions = (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.preconditions ?? [];
                            return {
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  preconditions: [...currentPreconditions, ""],
                                }),
                              },
                            };
                          });
                        }}
                      >
                        <Plus className="mr-1 h-3 w-3" />
                        条件を追加
                      </Button>
                    </TabsContent>

                    <TabsContent value="sideEffects" className="space-y-2">
                      {!normalizedRuleName && (
                        <p className="text-xs text-amber-700">
                          ルール名を入力すると、副作用を選択できます。
                        </p>
                      )}
                      {sideEffectSelections.length === 0 ? (
                        <p className="text-xs text-muted-foreground">
                          副作用は定義されていません。副作用セクションで追加してください。
                        </p>
                      ) : (
                        <>
                          {normalizedRuleName && (
                            <p className="text-[11px] text-slate-500">
                              このルールに {linkedSideEffectCount} 件の副作用が紐付いています。
                            </p>
                          )}
                          <div className="space-y-2">
                            {sideEffectSelections.map((item) => {
                              const isChecked = Boolean(
                                normalizedRuleName && item.ruleRef === normalizedRuleName
                              );
                              const assignedOtherRule =
                                item.ruleRef && item.ruleRef !== normalizedRuleName;

                              return (
                                <div
                                  key={item.key}
                                  className="flex items-center gap-2 rounded-md border border-slate-200 px-2 py-1.5"
                                >
                                  <Checkbox
                                    checked={isChecked}
                                    disabled={!normalizedRuleName}
                                    onCheckedChange={(checked) => {
                                      if (!normalizedRuleName) return;
                                      setSideEffectRuleRef(
                                        item.kind,
                                        item.index,
                                        checked === true ? normalizedRuleName : undefined
                                      );
                                    }}
                                  />
                                  <Badge variant="outline" className="h-5 px-2">
                                    {getSideEffectKindLabel(item.kind)}
                                  </Badge>
                                  <span className="flex-1 text-xs text-slate-700">{item.label}</span>
                                  {assignedOtherRule && (
                                    <span className="text-[11px] text-amber-700">
                                      現在: {item.ruleRef}
                                    </span>
                                  )}
                                </div>
                              );
                            })}
                          </div>
                        </>
                      )}
                    </TabsContent>

                    <TabsContent value="notes" className="space-y-2">
                      {(rule.notes || []).map((note, noteIndex) => (
                        <div key={noteIndex} className="flex items-center gap-2">
                          <Input
                            value={note}
                            onChange={(e) => {
                              updateStructuredSpec((current) => ({
                                ...current,
                                coreLogic: {
                                  ...current.coreLogic,
                                  rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                    notes: updateAtIndex(
                                      (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.notes,
                                      noteIndex,
                                      e.target.value
                                    ),
                                  }),
                                },
                              }));
                            }}
                            placeholder="補足情報・備考"
                          />
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => {
                              updateStructuredSpec((current) => ({
                                ...current,
                                coreLogic: {
                                  ...current.coreLogic,
                                  rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                    notes: removeAtIndex(
                                      (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.notes,
                                      noteIndex
                                    ),
                                  }),
                                },
                              }));
                            }}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      ))}
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          updateStructuredSpec((current) => {
                            const currentNotes = (current.coreLogic.rules?.[ruleIndex] as BusinessRule)?.notes ?? [];
                            return {
                              ...current,
                              coreLogic: {
                                ...current.coreLogic,
                                rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                  notes: [...currentNotes, ""],
                                }),
                              },
                            };
                          });
                        }}
                      >
                        <Plus className="mr-1 h-3 w-3" />
                        備考を追加
                      </Button>
                    </TabsContent>

                    <TabsContent value="sequence" className="space-y-2">
                      <div className="rounded-md border border-slate-200 p-3">
                        <div className="overflow-x-auto pb-1">
                          <div className="grid w-full min-w-[980px] gap-3 grid-cols-[15%_20%_15%_25%_1fr] items-start">
                            <div className="space-y-1">
                              <Label className="text-[11px] text-slate-500">フラグメント</Label>
                              <Select
                                value={rule.sequence?.fragmentType ?? EMPTY_SELECT_VALUE}
                                onValueChange={(value) => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    const fragmentType = value === EMPTY_SELECT_VALUE
                                      ? undefined
                                      : (value as RuleSequence["fragmentType"]);
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                          sequence: patchRuleSequence(currentRule.sequence, {
                                            fragmentType,
                                            branch: undefined,
                                            loopLabel: undefined,
                                          }),
                                        }),
                                      },
                                    };
                                  });
                                }}
                              >
                                <SelectTrigger className="h-8 w-full text-sm">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={EMPTY_SELECT_VALUE}>未指定</SelectItem>
                                  <SelectItem value="alt">alt（条件分岐）</SelectItem>
                                  <SelectItem value="opt">opt（任意分岐）</SelectItem>
                                  <SelectItem value="loop">loop（繰り返し）</SelectItem>
                                  <SelectItem value="par">par（並列）</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] text-slate-500">グループID（任意）</Label>
                              <Input
                                value={rule.sequence?.fragmentGroup ?? ""}
                                onChange={(e) => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                          sequence: patchRuleSequence(currentRule.sequence, {
                                            fragmentGroup: e.target.value,
                                          }),
                                        }),
                                      },
                                    };
                                  });
                                }}
                                placeholder="例: auth_check"
                                className="h-8 text-sm"
                              />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] text-slate-500">分岐（alt専用）</Label>
                              <Select
                                value={rule.sequence?.branch ?? EMPTY_SELECT_VALUE}
                                onValueChange={(value) => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                          sequence: patchRuleSequence(currentRule.sequence, {
                                            branch: value === EMPTY_SELECT_VALUE
                                              ? undefined
                                              : (value as RuleSequence["branch"]),
                                          }),
                                        }),
                                      },
                                    };
                                  });
                                }}
                              >
                                <SelectTrigger className="h-8 w-full text-sm">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={EMPTY_SELECT_VALUE}>未指定</SelectItem>
                                  <SelectItem value="if">if</SelectItem>
                                  <SelectItem value="else">else</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] text-slate-500">ガード条件（任意）</Label>
                              <Input
                                value={rule.sequence?.guard ?? ""}
                                onChange={(e) => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                          sequence: patchRuleSequence(currentRule.sequence, {
                                            guard: e.target.value,
                                          }),
                                        }),
                                      },
                                    };
                                  });
                                }}
                                placeholder="例: 金額 > 与信枠"
                                className="h-8 text-sm"
                              />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] text-slate-500">loopラベル（loop専用）</Label>
                              <Input
                                value={rule.sequence?.loopLabel ?? ""}
                                onChange={(e) => {
                                  updateStructuredSpec((current) => {
                                    const currentRule = (current.coreLogic.rules?.[ruleIndex] as BusinessRule) ?? rule;
                                    return {
                                      ...current,
                                      coreLogic: {
                                        ...current.coreLogic,
                                        rules: updateAtIndex(current.coreLogic.rules, ruleIndex, {
                                          sequence: patchRuleSequence(currentRule.sequence, {
                                            loopLabel: e.target.value,
                                          }),
                                        }),
                                      },
                                    };
                                  });
                                }}
                                placeholder="例: 明細行ごと"
                                className="h-8 text-sm"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </TabsContent>
                  </Tabs>
                </div>
                );
              })}
            </div>
          ) : (
            <div className="text-sm text-muted-foreground italic py-2">
              ビジネスルールが定義されていません
            </div>
          )}
        </div>
      </div>
    </FoldableStructuredSection>
  );
}
