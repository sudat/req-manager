...
/**
 * "legacy" を大文字小文字区別なしで削除（単語境界）
 *
 * 処理例:
 * - "legacy requirement" → "requirement"
 * - "This is legacy text" → "This is text"
 * - "Legacy System" → "System"
 * - "LEGACY data" → "data"
 * - "legacy data migration" → "data migration"
 */
...
const main = async () => {
  ...
  try {
    // eslint-disable-next-line no-console
    console.log("==> Scanning for 'legacy' text...");
    ...
    if (remainingCount > 0) {
      // eslint-disable-next-line no-console
      console.warn(`\n==> Warning: ${remainingCount} records still contain 'legacy'`);
    } else {
      // eslint-disable-next-line no-console
      console.log("\n==> Verification: All 'legacy' text removed");
    }
  } finally {
    await client.end();
  }
