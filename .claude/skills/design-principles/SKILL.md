...
description: Linear、Notion、Stripeにインスパイアされた、正確でミニマルなデザインシステムを適用します。Jony Iveレベルの精度が求められるダッシュボード、管理インターフェース、またはクリーンでモダン、洗練されたミニマリズムを必要とするUIを構築する際に、このスキルを使用します。すべてのピクセルが重要です。Enforce a precise, minimal design system inspired by Linear, Notion, and Stripe. Use this skill when building dashboards, admin interfaces, or any UI that needs Jony Ive-level precision - clean, modern, minimalist with taste. Every pixel matters.
---
